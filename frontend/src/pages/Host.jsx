...
const Host = () => {
  ...
  const updateGameState = () => {
    socket.emit('updateGameState', { question, answers, teamAScore, teamBScore, strikes, activeTeam });
  };
  ...
  const revealAnswer = (index) => {
    ...
    socket.emit('updateGameState', { question, answers: newAnswers, teamAScore: newTeamAScore, teamBScore: newTeamBScore, strikes, activeTeam });
  };
  ...
  const hideAnswer = (index) => {
    ...
    setAnswers(newAnswers);
    socket.emit('updateGameState', { question, answers: newAnswers, teamAScore, teamBScore, strikes, activeTeam });
  };
  ...
  const handleStrike = () => {
    ...
    socket.emit('updateGameState', { question, answers, teamAScore, teamBScore, strikes: newStrikes, activeTeam: newActiveTeam });
  };
  
  const handleResetStrikes = () => {
    setStrikes(0);
    socket.emit('updateGameState', { question, answers, teamAScore, teamBScore, strikes: 0, activeTeam });
  }

  const handleActiveTeamChange = (team) => {
    setActiveTeam(team);
    socket.emit('updateGameState', { question, answers, teamAScore, teamBScore, strikes, activeTeam: team });
  }
  ...
  const handleSteal = (stealingTeam) => {
    if (stealingTeam === 'B') {
        ...
        socket.emit('updateGameState', { question, answers, teamAScore: newTeamAScore, teamBScore: newTeamBScore, strikes, activeTeam });
    ...
    <div className="p-4 bg-gray-900 text-white">
      ...
      <div className="flex justify-around">
        <div className="text-center">
            <h2 className="text-xl mb-2">Team A: {teamAScore} (Round: {roundScore.A})</h2>
            <button 
                onClick={() => handleActiveTeamChange('A')} 
                className={`p-2 rounded ${activeTeam === 'A' ? 'bg-green-500' : 'bg-blue-500'}`}
            >
                Set Active
            </button>
            <button onClick={() => handleSteal('A')} className="p-2 bg-orange-500 rounded ml-2">Steal</button>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
            <h2 className="text-xl mb-2">Team B: {teamBScore} (Round: {roundScore.B})</h2>
            <button 
                onClick={() => handleActiveTeamChange('B')} 
                className={`p-2 rounded ${activeTeam === 'B' ? 'bg-green-500' : 'bg-blue-500'}`}
            >
                Set Active
            </button>
            <button onClick={() => handleSteal('B')} className="p-2 bg-orange-500 rounded ml-2">Steal</button>
        </div>
      </div>
